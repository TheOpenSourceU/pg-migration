module.exports = function (dbConnection, migrations) {
  ...
  return processVersion(migrations['1.0.0']).then(function (rObj){
    ...
    $npm.debug('[then2] rObj=', rObj);
    if(!rObj.result) throw new Error("unexpected result in test harness part 2");
    //pg-promise documents say it there is a limitation to bulk for promises (has othe API)
    return processVersion(migrations['1.0.2']);
  });
};